...
import SellectOrganization from "../SelectOrganization";

interface IIAuthUser extends ICurrentUser {
  affectation: any[];
}
...
function Login() {
  ...
  const [dataUser_, setDataUser] = useState<any>([]);
  const [affectationsUsserLogged, setAffectationsUsserLogged] = useState<any[]>(
    []
  );
  ...
  const handleSubmitLogin = async () => {
    ...
    try {
      ...
      const { data } = await postAPI<IFetchData<IIAuthUser>, ILogin>(
        "login",
        formLogin
      );
      if (data.status === 1 && data.data) {
        ...
        //=========================
        setDataUser(data);
        setAffectationsUsserLogged(data.data?.affectation || []);
        //=========================
  ...
  const selectOrg = (affecation_: any) => {
    const data = { ...dataUser_ };
    let permissions_ = getPermissionsofCurrentUser(
      data.data as any as unknown as ICurrentUserPermission
    );
    const affecationSelected_ = permissions_.filter(
      (item: any) => item.id === affecation_.id
    );
    let data_ = {
      ...data.data,
      metaData: {
        permissions: permissions_,
        affectationSelected: affecationSelected_,
      },
    };
  ...
  return (
    <motion.div animate={{ height: "fit-content" }}>
      <>
        {affectationsUsserLogged.length === 0 ? (
          <>
                ...
                >
                  ...
                  <div className="w-2/5 md:block hidden  bg-main-color-dark text-white rounded-tr-2xl rounded-br-2xl py-36 px-12">
                    ...
                    <p className="text-xs mb-2">
                      Bienvenue sur la plateforme <strong>AfiaGap</strong>. Un
                      outil du monitoring de la préparation et la réponse aux
                      urgences
                    </p>
                  </div>
                </main>
